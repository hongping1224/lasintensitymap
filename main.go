...
func SaveLabelTiff(filename string, DensityMap []uint32, w, h int, gap float64, LB counter.Point) {
	//convert to 2d array
	...
	for y := h - 1; y > -1; y-- {
		for x := 0; x < w; x++ {
			i := x + (y * w)
			im.SetGray32(x, h-1-y, tiff32.Gray32Color{Y: DensityMap[i]})
		}
	}
	//write tfw file
	fmt.Println("Saving Tiff file at:", filename)
	f, err := os.Create(filename + "_Mean_Intensity.tiff")
	if err != nil {
		log.Fatal(err)
		return
	}
	...
	f.Close()
	tfw, err := os.Create(filename + "_Mean_Intensity.tfw")
...
func Calculate(lf *lidario.LasFile, MinX, MinY, MaxX, MaxY, gap float64) ([]uint32, int, int) {
	...
	for j := 1; j < cpu; j++ {
		for i := 0; i < mapsize; i++ {
			...
			counters[0].IntensitMap[i] += counters[j].IntensitMap[i]
		}
		counters[j] = nil
		runtime.GC()
	}
	meanIntensity := make([]uint32, len(counters[0].IntensitMap))
	for i := 0; i < mapsize; i++ {
		meanIntensity[i] = 0
		if counters[0].DensityMap[i] != 0 {
			meanIntensity[i] = counters[0].IntensitMap[i] / counters[0].DensityMap[i]
		}
	}
	return meanIntensity, counters[0].XRange, counters[0].YRange
}
